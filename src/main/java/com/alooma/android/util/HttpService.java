...
/**
 * An HTTP utility class for internal use in the Alooma library. Not thread-safe.
 */
public class HttpService implements RemoteService {

    private static boolean sIsAloomaBlocked;
    private static final int MIN_UNAVAILABLE_HTTP_RESPONSE_CODE = HttpURLConnection.HTTP_INTERNAL_ERROR;
    private static final int MAX_UNAVAILABLE_HTTP_RESPONSE_CODE = 599;

    @Override
    public void checkIsAloomaBlocked() {
    ...
    public boolean isOnline(Context context, OfflineMode offlineMode) {
        if (sIsAloomaBlocked) return false;
        ...
        try {
            ...
            if (netInfo == null) {
                isOnline = true;
                ALLog.v(LOGTAG, "A default network has not been set so we cannot be certain whether we are offline");
            } else {
                isOnline = netInfo.isConnectedOrConnecting();
                ALLog.v(LOGTAG, "ConnectivityManager says we " + (isOnline ? "are" : "are not") + " online");
            }
        } catch (final SecurityException e) {
            isOnline = true;
            ALLog.v(LOGTAG, "Don't have permission to check connectivity, will assume we are online");
        }
        return isOnline;
    }
    ...
    private boolean onOfflineMode(OfflineMode offlineMode) {
        ...
        try {
            ...
            onOfflineMode = false;
            ALLog.v(LOGTAG, "Client State should not throw exception, will assume is not on offline mode", e);
        }
    ...
    public byte[] performRequest(String endpointUrl, Map<String, Object> params, SSLSocketFactory socketFactory) throws ServiceUnavailableException, IOException {
        ALLog.v(LOGTAG, "Attempting request to " + endpointUrl);
        ...
        while (retries < 3 && !succeeded) {
            ...
            try {
                ...
                ALLog.d(LOGTAG, "Failure to connect, likely caused by a known issue with Android lib. Retrying.");
                retries = retries + 1;
        ...
        if (retries >= 3) {
            ALLog.v(LOGTAG, "Could not connect to Alooma service after three retries.");
        }
        return response;
    }
    ...
    private static final String LOGTAG = "AloomaAPI.Message";
}
