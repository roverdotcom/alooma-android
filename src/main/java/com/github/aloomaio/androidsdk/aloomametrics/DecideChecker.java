...
/* package */ class DecideChecker {
    ...
    public DecideChecker(final Context context, final AConfig config) {
        mContext = context;
        mConfig = config;
    ...
    /* package */ static Result parseDecideResponse(String responseString)
        ...
        if (null != notifications) {
            final int notificationsToRead = Math.min(notifications.length(), AConfig.MAX_NOTIFICATION_CACHE_COUNT);
